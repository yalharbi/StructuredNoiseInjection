...
#----------------------------------------------------------------------------
# A generator utilizing structured noise injection which combines a spatially-variable code and a style code
...
def G_mapping(
    ...
    local_latent_size       = 8,            # Dimensionality of each local cell code
    local_cell_num          = 8,              # Number of cells in one row/column of local codes (default is 8 to receive 8x8 local codes)
    shared_cell_num         = 2,             # Number of cells in one row/column of shared codes (default is 2 to receive 2x2 shared codes)
    shared_latent_size         = 1,             # Dimensionality of each shared cell code
    global_code_size        = 1,             # Size of the globally shared entry (default is 1)
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    # Inputs.
    styleCodeSize = 512 #dlatent_size
    spatiallyvariablyCodeSize = 1 + 2*2 + 8*8*16 + 3#1032 #global_code_size + shared_cell_num*shared_cell_num*shared_latent_size + local_cell_num*local_cell_num*local_latent_size
    
    latents_in.set_shape([None, styleCodeSize+spatiallyvariablyCodeSize])
    labels_in.set_shape([None, label_size])
    latents_in = tf.cast(latents_in, dtype)
    labels_in = tf.cast(labels_in, dtype)
    styleCode, spatiallyVariableCode = tf.split(latents_in, [styleCodeSize, spatiallyvariablyCodeSize], axis=1) 
    ...
    if label_size:
        with tf.variable_scope('LabelConcat'):
            ...
            styleCode = tf.concat([styleCode, y], axis=1)

    # Normalize latents.
    if normalize_latents:
        styleCode= pixel_norm(styleCode)
        spatiallyVariableCode= pixel_norm(spatiallyVariableCode) 


    # Mapping layers for the style code.
    x1_out = styleCode
    for layer_idx in range(mapping_layers):
        with tf.variable_scope('Dense%d' % layer_idx):
            fmaps = dlatent_size if layer_idx == mapping_layers - 1 else mapping_fmaps
            x1_out = dense(x1_out, fmaps=fmaps, gain=gain, use_wscale=use_wscale, lrmul=mapping_lrmul)
            x1_out = apply_bias(x1_out, lrmul=mapping_lrmul)
            x1_out = act(x1_out)
    ...
    structuredCodes = tf.reshape(structuredCodes, [-1, 8*8*structuredCodeLength]) # StructuredCodes contains the structured code for each cell as described in the paper
    
    # Mapping layers for the spatially-variable code.
    with tf.variable_scope('LocationMapping%d' % 0):
		            
        cellCode = structuredCodes[:, 0:structuredCodeLength]
				
        mappedCell = dense(cellCode, fmaps=512, gain=gain, use_wscale=use_wscale) #This is responsible for the input tensor information at cell (0,0)
        mappedCell = tf.reshape(mappedCell, [-1, 512, 1])
        allMappedCells = mappedCell
		   
    for ii in range(1,64):
        with tf.variable_scope('LocationMapping%d' % ii):
            cellCode = structuredCodes[:, ii*structuredCodeLength:(ii+1)*structuredCodeLength]

            mappedCell = dense(cellCode, fmaps=512, gain=gain, use_wscale=use_wscale)
            mappedCell = tf.reshape(mappedCell, [-1, 512, 1])

            allMappedCells = tf.concat([allMappedCells, mappedCell], axis=2)
					
					
    allMappedCells = tf.reshape(allMappedCells, [-1, 512, 8, 8])
    

    with tf.variable_scope('spatiallyVariableMapping'):

        allMappedCells = (act(apply_bias(allMappedCells)))
	
    allMappedCells = tf.reshape(allMappedCells, [-1, 512* 8* 8])
    mappedSpatiallyvariableCode = allMappedCells
			
    # Broadcast.
    if dlatent_broadcast is not None:
        with tf.variable_scope('Broadcast'):
            mappedStyleCode = tf.tile(mappedStyleCode[:, np.newaxis], [1, dlatent_broadcast, 1])
            mappedSpatiallyvariableCode = tf.tile(mappedSpatiallyvariableCode[:, np.newaxis], [1, dlatent_broadcast, 1]) #Currently unused since this code is not mixed during training

    # Output.
	
    assert mappedStyleCode.dtype == tf.as_dtype(dtype)
    x = tf.concat([mappedStyleCode, mappedSpatiallyvariableCode], axis = 2)
...
def G_synthesis(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    # Primary inputs.
    dlatents_in.set_shape([None, num_styles, dlatent_size+dlatent_size*8*8])    
    dlatents_in = tf.cast(dlatents_in, dtype)
    dlatents_in1, dlatents_in2 = tf.split(dlatents_in, [512, 512*8*8], axis=2)
    ...
    def layer_epilogue(x, layer_idx):
        ...
        if use_instance_norm:
            x = instance_norm(x)
        if use_styles and layer_idx > 4: #Disentanglement results are better if style is not applied at the first few layers
            x = style_mod(x, dlatents_in1[:, layer_idx], use_wscale=use_wscale)
        return x

    # Early layers.
    with tf.variable_scope('4x4'):
        x = tf.zeros_like(downscale2d(tf.reshape(dlatents_in2[:,0,:], [-1, 512, 8, 8]))) #Unused as we start from the 8x8 resolution
    ...
    def block(res, x): # res = 3..resolution_log2
      ...
      if res == 3: # 8x8 and up
        with tf.variable_scope('8x8'):
                x  = instance_norm(tf.reshape(dlatents_in2[:,0,:], [-1, 512, 8, 8])) #The spatially-variable code replaces the constant input in StyleGAN
                return x
