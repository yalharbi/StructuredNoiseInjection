...
#----------------------------------------------------------------------------
# This is used during training to showcase different results (randomizing style, pose, hair, mouth, eyebrows)
def my_randoms(latent_size, gh, gw):
    allCodes = np.zeros(shape=(gw, gh, latent_size)).astype(np.float32)
    for j in range(gw):
        allCodes[j,:, :] = np.random.randn(latent_size).astype(np.float32)
        for i in range(5):
          allCodes[j, i+0, 0:512] =  np.random.randn(512).astype(np.float32) 
          
        for i in range(5):
          allCodes[j, i+5, 512:512+1] =  np.random.randn(1).astype(np.float32)

        for i in range(5):
          allCodes[j, i+15, 512+8:] =  np.random.randn(16*8*8).astype(np.float32) 
          
        for i in range(5):
          allCodes[j, i+20, 8+512+(43*16):8+512+(45*16)] =  np.random.randn(16*2).astype(np.float32) 
          allCodes[j, i+20, 8+512+(51*16):8+512+(53*16)] =  np.random.randn(16*2).astype(np.float32) 

		
        for i in range(5):
          allCodes[j, i+25, 8+512+(26*16):8+512+(30*16)] =  np.random.randn(16*4).astype(np.float32) 
          allCodes[j, i+25, 8+512+((26+8)*16):8+512+((30+8)*16)] =  np.random.randn(16*4).astype(np.float32) 

          
        for i in range(5):
          allCodes[j, i+30, 8+512+(1*16):512+(16*7)+8] =  np.random.randn(6*16).astype(np.float32) 
          allCodes[j, i+30, 8+512+(9*16):512+(15*16)+8] =  np.random.randn(6*16).astype(np.float32) 
          allCodes[j, i+30, 8+512+(17*16):512+(23*16)+8] =  np.random.randn(6*16).astype(np.float32) 

        for i in range(5):
          allCodes[j, i+10, 512+1:512+5] =  np.random.randn(4).astype(np.float32)

          
    return np.reshape(allCodes, (gw*gh, latent_size)).astype(np.float32)
    
def randomize_global_codes(latents, number_variations):
    latent_size = latents.shape[1]
    num_faces = latents.shape[0]

    latents = latents[:,np.newaxis,:]
    allCodes = np.tile(latents, (1, number_variations, 1))   #np.zeros(shape=(gw, gh, latent_size)).astype(np.float32)
    for j in range(num_faces):
        for i in range(number_variations):
          allCodes[j, i+0, 512:512+1] =  np.random.randn(1).astype(np.float32) 
          
          
    return np.reshape(allCodes, (-1, latent_size)).astype(np.float32)
    
def randomize_global_shared_codes(latents, number_variations):
    latent_size = latents.shape[1]
    num_faces = latents.shape[0]
    ...
    for j in range(num_faces):
        for i in range(number_variations):
          allCodes[j, i+0, 512+1:512+5] =  np.random.randn(2*2).astype(np.float32) 
          
          
    return np.reshape(allCodes, (-1, latent_size)).astype(np.float32)
    
def randomize_all_local_codes(latents, number_variations):
    latent_size = latents.shape[1]
    num_faces = latents.shape[0]
    ...
    for j in range(num_faces):
        for i in range(number_variations):
          allCodes[j, i+0, 512+8:] =  np.random.randn(16*8*8).astype(np.float32) 
          
          
    return np.reshape(allCodes, (-1, latent_size)).astype(np.float32)
    
def randomize_specific_local_codes(latents, modification_mask, number_variations):
    latent_size = latents.shape[1]
    num_faces = latents.shape[0]
    ...
    for j in range(num_faces):
        for i in range(number_variations):
          newCodes =  np.random.randn(8, 8, 16).astype(np.float32) 
          oldCodes = np.reshape(allCodes[j, i+0, 512+8:], [8,8,16])
          
          
          allCodes[j, i+0, 512+8:] =  np.reshape( oldCodes * (1-modification_mask) + newCodes * modification_mask, [1, 1, 8*8*16])
...
def setup_snapshot_image_grid(G, training_set,
    ...
    gw = 1; gh = 1
    if size == '1080p':
        gw = 35#np.clip(1920 // G.output_shape[3], 3, 32)
        gh = 4#np.clip(1080 // G.output_shape[2], 2, 32)
    if size == '4k':
        gw = 35#np.clip(3840 // G.output_shape[3], 7, 32)
        gh = 4#np.clip(2160 // G.output_shape[2], 4, 32)

    # Initialize data arrays.
    reals = np.zeros([gw * gh] + training_set.shape, dtype=training_set.dtype)
    labels = np.zeros([gw * gh, training_set.label_size], dtype=training_set.label_dtype)
    latents = my_randoms(*G.input_shape[1:], gw, gh) #np.random.randn(gw * gh, *G.input_shape[1:])
